...
class swin_former(nn.Module):

    def __init__(self, opts, img_size=224, patch_size=4, in_chans=3,
        ...
        self.num_layers = len(opts.DEPTHS)
        self.embed_dim = opts.EMBED_DIM
        self.depths = opts.DEPTHS
        ...
        self.patch_embed_reversed = nn.Sequential(
            ...
            nn.Conv2d(self.embed_dim, 64, kernel_size=5, stride=1, padding=2),
            LayerNorm(64),
            nn.LeakyReLU(0.2),
            nn.Upsample(scale_factor=2,align_corners=False,mode='bilinear'),
        ...
        for i_layer in range(self.num_layers):
            ...
            self.transformer_encoder.append(layer)

        self.transformer_decoder = nn.ModuleList()
        for j_layer in reversed(range(self.num_layers)):
            de_layer = BasicLayer(dim=int(self.embed_dim * 2 ** j_layer),
                               depth=self.depths[j_layer],
                               num_heads=self.num_heads[j_layer],
                               window_size=window_size,
                               mlp_ratio=self.mlp_ratio,
                               qkv_bias=qkv_bias, qk_scale=qk_scale,
                               drop=drop_rate, attn_drop=attn_drop_rate,
                               drop_path=dpr[sum(self.depths[:j_layer]):sum(self.depths[:j_layer + 1])],
                               norm_layer=norm_layer,
                               downsample=PatchMerging_reverse if (j_layer > 0) else None,
                               use_checkpoint=use_checkpoint, mode='decode')
            self.transformer_decoder.append(de_layer)
    ...
    def get_last_layer(self):
        return self.patch_embed_reversed[-1].weight
    ...
    def decode_features(self, x, shortcut_x):
        ...
        for layer in self.transformer_decoder:
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww)
            if i < 3:
                mask = torch.ones(size=[Wh, Ww]).long()
                pad_width = 2**(i+1)
                mask[pad_width:-pad_width, pad_width:-pad_width] = 0
                mask = mask.view(-1)

                merge = (x[:,mask==0,:] + shortcut_x[2-i])/2
                x[:, mask==0,:] = merge
                i = i + 1
    ...
    def forward(self, samples):
        ...
        B = x.size(0)
        x, shortcut_x = self.encode_features(x)
        x = self.feature_expand(x)
        out = self.decode_features(x, shortcut_x)
        p=(192-128)//2
        out[:, :, p:-p, p:-p] = gt_inner[:, :, p:-p, p:-p]
        return out
...
class LayerNorm2(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size(0) == 1:
            ...
            mean = x.reshape(x.size(0), -1).mean(1).view(*shape)
            std = x.reshape(x.size(0), -1).std(1).view(*shape)
        x = (x - mean) / (std + self.eps)
        if self.affine:
            shape = [1, -1] + [1] * (x.dim() - 2)
            x = x * self.gamma.view(*shape) + self.beta.view(*shape)
